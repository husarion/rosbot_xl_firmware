...
public:
    rcl_ret_t init(rcl_node_t& node, const char* topic_name, rcl_allocator_t& allocator) {
        initMsg(allocator);
...
private:
    ...
    void initMsg(rcl_allocator_t& allocator) {
        ...
        msg_.position.data =
            (double*)allocator.allocate(num_of_joints * sizeof(double), allocator.state);
        ...
        // msg_.effort.data = (double*)allocator.allocate(num_of_joints * sizeof(double),
        // allocator.state);
