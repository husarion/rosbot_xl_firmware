...
UartProtocolClass::UartProtocolClass(uint32_t arg_rx, uint32_t arg_tx,
                                     uint32_t arg_baudrate, uint8_t arg_config)
...
int8_t UartProtocolClass::StreamParse() {
  ...
  for (uint16_t i = 0; i < rx_buffer_size || i < RX_BUFF_CAPACITY; i++) {
    if (rx_buffer[i] == '<') {
      counted_check_sum = 0;
      if (HexToByte(&rx_buffer[i + 1], &this->processed_frame.cmd) !=
          ConversionError) {
...
UartConvStatusTypeDef UartProtocolClass::HexToByte(uint8_t* arg_byte,
                                                   uint8_t* arg_result) {
  uint8_t value;
  ...
  switch (this->processed_frame.cmd) {
    ...
    case 2: {  // Battery State
      ...
      battery_state.voltage = (float(this->processed_frame.args[1] << 8 |
                              ...
                              0.001;
      int32_t temperature =
          (this->processed_frame.args[3] << 8 | this->processed_frame.args[4]);
      if (temperature > 200 || temperature < -100) {
        battery_state.temperature = NAN;
      } else
        battery_state.temperature = (float)temperature;
      battery_state.current = (float((this->processed_frame.args[7] << 8 |
                                      this->processed_frame.args[8]) -
                                     (this->processed_frame.args[5] << 8 |
                                      this->processed_frame.args[6]))) *
                              0.001;
      ...
      battery_state.design_capacity =
          (float(this->processed_frame.args[11] << 8 |
                 this->processed_frame.args[12])) *
          0.001;
      // battery_state.percentage = this->processed_frame.args[13];
      battery_state.percentage = NAN;
      battery_state.status =
          (BatteryStatusTypeDef)this->processed_frame.args[14];
      battery_state.health =
          (BatteryHealthTypeDef)this->processed_frame.args[15];
      battery_state.technology =
          (BatteryTechnologyTypeDef)this->processed_frame.args[16];
      ...
      xQueueOverwrite(rtos::queues::BatteryStateQueue,
                        static_cast<void*>(&battery_state));
    } break;
    default:
      break;
  }
...
void UartProtocolClass::SendBuffer(uint8_t arg_size, uint8_t* arg_buffer) {
  String input = reinterpret_cast<char*>(arg_buffer);
