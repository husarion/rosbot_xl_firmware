// encoder.cpp - Hardware Timer Encoder Implementation
#include "encoder.hpp"
...
void Encoder::init(uint8_t pin_a, uint8_t pin_b, TIM_TypeDef* timer, Direction dir, float rad_per_tick) {
    ...
    if (timer == TIM1 || timer == TIM2) af = GPIO_AF1_TIM1;
    else if (timer == TIM8) af = GPIO_AF3_TIM8;
    ...
    encoder_config_.IC1Polarity = (dir == Direction::CW) ? TIM_ICPOLARITY_RISING : TIM_ICPOLARITY_FALLING;
