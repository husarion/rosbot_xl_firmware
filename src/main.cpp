...
void setup()
{
  ...
  /* RTOS TASKS CREATION */
  s1 = xTaskCreate(
    RclcSpinTask, "RclcSpinTask", configMINIMAL_STACK_SIZE + 3000, NULL, tskIDLE_PRIORITY + 1,
    NULL);
  if (s1 != pdPASS)
    if (firmware_mode == fw_debug) Serial.printf("S1 creation problem\r\n");
  s2 = xTaskCreate(
    ImuTask, "ImuTask", configMINIMAL_STACK_SIZE + 1000, NULL, tskIDLE_PRIORITY + 1, NULL);
  if (s2 != pdPASS)
    if (firmware_mode == fw_debug) Serial.printf("S2 creation problem\r\n");
  s3 = xTaskCreate(
    RuntimeStatsTask, "RuntimeStatsTask", configMINIMAL_STACK_SIZE + 1000, NULL,
  ...
  s5 = xTaskCreate(
    PixelLedTask, "PixelLedTask", configMINIMAL_STACK_SIZE + 1000, NULL, tskIDLE_PRIORITY + 1,
    NULL);
  if (s5 != pdPASS)
  ...
  s9 = xTaskCreate(
    uRosPingTask, "uRosPingTask", configMINIMAL_STACK_SIZE + 100, NULL, tskIDLE_PRIORITY + 1, NULL);
  if (s9 != pdPASS)
