    ...
    while(1) {
        ...
        if(freq_div_ptr > (PID_FREQ / MOTORS_RESPONSE_FREQ)) {
            for(int i=0;i<4;i++) {
                ...
                motor_state.position[i] = wheel_motors[i].GetWheelAbsPosition()/1000.0;
            }
...
void create() {
    auto result = xTaskCreate(task, "RclcSpinTask", configMINIMAL_STACK_SIZE + 2500,
                              nullptr, tskIDLE_PRIORITY + 1, &handle);
    if(result != pdPASS) {
...
void create() {
    auto result = xTaskCreate(task, "RuntimeStatsTask", configMINIMAL_STACK_SIZE + 500,
                              nullptr, tskIDLE_PRIORITY + 1, &handle);
    if(result != pdPASS) {
...
void create() {
    auto result = xTaskCreate(task, "URosPingTask", configMINIMAL_STACK_SIZE + 500,
                              nullptr, tskIDLE_PRIORITY + 1, &handle);
    if(result != pdPASS) {
