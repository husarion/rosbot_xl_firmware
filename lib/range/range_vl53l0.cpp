...
void RangeVl53l0x::init() {
    ...
    driver_.setVcselPulsePeriod(VL53L0X::VcselPeriodPreRange, config_.vcsel_pre_range);
    driver_.setVcselPulsePeriod(VL53L0X::VcselPeriodFinalRange, config_.vcsel_final_range);
...
void RangeVl53l0x::update() {
    ...
    data_.range = driver_.timeoutOccurred() ? NAN : static_cast<float>(range_mm) / 1000.0f;
}
